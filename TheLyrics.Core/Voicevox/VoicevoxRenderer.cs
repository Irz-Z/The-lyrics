...
using TheLyrics.Core.Format;
using TheLyrics.Core.Render;
...
namespace TheLyrics.Core.Voicevox {
            ...
            if (IsPhonemeNoteCountMatch(phrase) && phrase.phones.All(p => VoicevoxUtils.phoneme_List.kanas.ContainsKey(p.phoneme))) {
                ...
                if (vsParams.phonemes.Count == vsParams_1.phonemes.Count) {
                    for (int i = 0; i < vsParams_1.phonemes.Count; i++) {
                        // TODO: Develop a VOICEVOX engine dedicated to TheLyrics so that synthesis parameters are updated when phonemes are changed.
